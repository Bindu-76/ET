...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_plan'])) {
    ...
    $amount_1 = $_POST['amount_1'] !== '' ? floatval($_POST['amount_1']) : null;
    $amount_3 = $_POST['amount_3'] !== '' ? floatval($_POST['amount_3']) : null;
    $amount_6 = $_POST['amount_6'] !== '' ? floatval($_POST['amount_6']) : null;
    ...
    if (!$message) {
        $stmt = $conn->prepare("INSERT INTO subscription_plans 
            (name, description, duration, amount_1, amount_3, amount_6, workshop, qr_code, razorpay_plan_id, amount_paise, currency, created_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
        ...
        $stmt->bind_param("sssddsisssis", 
            ...
            $razorpay_plan_id, $amount_paise, $currency);
...
<div class="container my-4">
    ...
    <form method="POST" enctype="multipart/form-data" action="manage_plans.php" class="mb-5">
        ...
        <div class="mb-3">
            ...
            <input type="text" id="duration" name="duration" class="form-control" required placeholder="e.g. 1 month, 3 months" />
        </div>

        <div class="mb-3">
            <label class="form-label">Prices (in your currency)</label>
            <div class="input-group mb-2">
                <span class="input-group-text">1 Month</span>
                <input type="number" step="0.01" min="0" name="amount_1" class="form-control" />
            </div>
            <div class="input-group mb-2">
                <span class="input-group-text">3 Months</span>
                <input type="number" step="0.01" min="0" name="amount_3" class="form-control" />
            </div>
            <div class="input-group mb-2">
                <span class="input-group-text">6 Months</span>
                <input type="number" step="0.01" min="0" name="amount_6" class="form-control" />
            </div>
        </div>
        ...
        <div class="mb-3">
            <label for="amount_paise" class="form-label">Amount in Paise (integer)</label>
            <input type="number" id="amount_paise" name="amount_paise" class="form-control" placeholder="e.g. 50000 for ₹500" />
        </div>
        ...
        <div class="mb-3">
            <label for="qr_code" class="form-label">Upload QR Code Image (optional)</label>
    ...
    <table class="table table-bordered align-middle">
        ...
        <?php foreach ($plans as $plan): ?>
            <tr>
                ...
                <td>
                    <?php if ($plan['qr_code'] && file_exists("uploads/qr_codes/" . $plan['qr_code'])): ?>
                        <img src="<?= "uploads/qr_codes/" . htmlspecialchars($plan['qr_code']) ?>" alt="QR Code" style="height:60px;">
                    <?php else: ?>
                        N/A
                    <?php endif; ?>
                </td>
                ...
                <td>
                    ...
                    <a href="manage_plans.php?delete=<?= $plan['id'] ?>" 
